...
public class BluetoothDataService extends Service {
    ...
    private static String MAC_ADDRESS; //"30:14:10:30:04:73";
        ...
        bluetoothIn = new Handler() {
            ...
            public void handleMessage(android.os.Message msg) {
                ...
                if (msg.what == handlerState) {                                     //if message is what we want
                    ...
                    receivedDataFromArduino.append(readMessage);
                    int endOfLineIndex = receivedDataFromArduino.indexOf("~"); //eol
                    if (endOfLineIndex > 0) {
                        String extractedData = receivedDataFromArduino.substring(0, endOfLineIndex).trim();
                        int dataLength = extractedData.length();
                        Log.v("RawLeng", Integer.toString(dataLength));
                        ...
                        if(dataLength==28) {
                            if (extractedData.charAt(0) == '#') {
                                ...
                                Log.v("Result: ", date + " T: " + temp + " V: " + volt + " A: " + amps);

                            }
                        }
                        receivedDataFromArduino.delete(0, receivedDataFromArduino.length());
                    }
...
private void checkBTState() {
        ...
        try {
            MAC_ADDRESS = sp.getString("btAddr",null);
...
private class ConnectingThread extends Thread {
    ...
    public void testSend(String input){
        Log.v("INPUT",input);
...
public class ConnectedThread extends Thread {
    ...
    public void run() {
        ...
        int bytes;
        bluetoothIn.sendEmptyMessage(0);
        // Keep looping to listen for received messages
        while (!stopThread) {
            try {
                bytes = mmInStream.read(buffer);            //read bytes from input buffer
                String readMessage = new String(buffer, 0, bytes);
                Log.d("DEBUG BT VAL", readMessage);
